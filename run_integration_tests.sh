...
sleep 1 && docker run -i --rm --name=database-service --net=database_net -e DATABASE_ADDRESS=mongodb-mock-database:27017 -e DATABASE_LISTEN_ADDRESS=0.0.0.0:80 database:$(git rev-parse --short HEAD) &
echo "testing database..."
sleep 1 && docker run --rm -i --name=database-test --net=database_net -e DATABASE_SERVICE_ADDRESS=database-service database-test:$(git rev-parse --short HEAD)
echo "cleaning up..."
docker stop mongodb-mock-database
docker stop database-service
docker network rm database_net
...
echo "testing auth..."
sleep 1 && docker run --rm -i --name=auth-test --net=auth_net -e AUTH_SERVICE_ADDRESS=auth-service auth-test:$(git rev-parse --short HEAD)
echo "cleaning up..."
docker stop auth-service
docker network rm auth_net
echo ""
...
docker run --rm -d --name=mongodb-mock-database --net=backend_net -e MONGO_INITDB_ROOT_USERNAME=adminz -e MONGO_INITDB_ROOT_PASSWORD=cheeksbutt mongodb-mock-database:$(git rev-parse --short HEAD)
sleep 1 && docker run -d --rm --name=database-service --net=backend_net -e DATABASE_ADDRESS=mongodb-mock-database:27017 -e DATABASE_LISTEN_ADDRESS=0.0.0.0:80 database:$(git rev-parse --short HEAD)
sleep 1 && docker run -i --rm --name=backend-service --net=backend_net -e DATABASE_ADDRESS=database-service -e BACKEND_LISTEN_ADDRESS=0.0.0.0:80 backend:$(git rev-parse --short HEAD) &
echo "testing backend..."
sleep 1 && docker run --rm -i --name=backend-test --net=backend_net -e BACKEND_API_SERVICE_ADDRESS=backend-service backend-test:$(git rev-parse --short HEAD)
echo "cleaning up..."
docker stop backend-service
docker stop database-service
docker stop mongodb-mock-database
...
sleep 1 && docker run --rm -i --name=frontend-api-test --net=frontend-api_net -e FRONTEND_API_SERVICE_ADDRESS=frontend-api-service frontend-api-test:$(git rev-parse --short HEAD)
echo "cleaning up..."
docker stop frontend-api-service
docker stop auth-service
docker stop backend-service
